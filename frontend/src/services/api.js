 ...
 * Prevents the UI from hanging forever when the backend is cold-starting or unresponsive.
 */
...
export async function fetchTool(slug) {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools/${slug}`, { credentials: 'include' })
  return checkRes(res)
}

export async function fetchCategories() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools/categories`, { credentials: 'include' })
  return checkRes(res)
}

export async function fetchPortalOverview() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/portal/overview`, { credentials: 'include' })
  return checkRes(res)
}
...
export async function fetchInternshipAutomationStatus() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/portal/internships/status`, { credentials: 'include' })
  return checkRes(res)
}

export async function refreshInternshipAutomation() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/portal/internships/refresh`, {
    method: 'POST',
    credentials: 'include',
...
export async function refreshAINewsAutomation() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/portal/news/refresh`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  }, WRITE_TIMEOUT_MS)
...
export async function bootstrapPortalContent() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/portal/bootstrap`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
  }, WRITE_TIMEOUT_MS)
...
export async function logout() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/auth/logout`, { method: 'POST', credentials: 'include' })
  return checkRes(res)
}

export async function fetchCurrentUser() {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/auth/me`, { credentials: 'include' })
  return checkRes(res)
}

export async function createTool(tool) {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools`, {
    method: 'POST',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
...
export async function importTools(payload, options = {}) {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools/bulk`, {
    method: 'POST',
    credentials: 'include',
...
export async function updateTool(slug, updates) {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools/${slug}`, {
    method: 'PUT',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteTool(slug) {
  requireBackendUrl()
  const res = await fetchWithTimeout(`${BACKEND}/api/tools/${slug}`, { method: 'DELETE', credentials: 'include' })
  return checkRes(res)
}
